...
class KFoldLoop(Loop):
    ...
    def on_advance_end(self) -> None:
        ...
        self.trainer.save_checkpoint(
            osp.join(self.trainer.weights_save_path, f"model_fold{self.current_fold}.pt")
        )
        ...
        if isinstance(loggers, LoggerCollection):
            for logger in loggers:
                if hasattr(logger, "increment"):
                    logger.increment()
        elif isinstance(loggers, LightningLoggerBase) and hasattr(loggers, "increment"):
            loggers.increment()
